"""
Helper module containing functions to set up data splits and train the BioBERT transformer with LLM sentences.
"""
import numpy as np
import torch
...
def get_split(split, dataset):
    ...
    df2 = pd.read_csv(os.path.join(dat_dir, dataset, "proc", "df_together_with_llm_sentence.csv"), index_col=0)
    split_df = pd.read_csv(os.path.join(dat_dir, dataset, "proc", "split.csv"), index_col=0)
    
    
    df = df[["label"]]
    df2 = df2[["llm_sentence"]]
